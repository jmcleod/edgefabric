...
export default function TenantAuditPage() {
  ...
  const columns: Column<AuditLogEntry>[] = [
    ...
    },
    {
      ...
      render: (log) => (
        <div>
          <p className="text-sm mono-data">{log.userEmail || log.userId}</p>
  ...
  return (
    <AppLayout breadcrumbs={[{ label: 'Access' }, { label: 'Audit Logs' }]}>
      <PageHeader
        title="Audit Logs"
        description="Activity history for your tenant"
        icon={FileText}
      />
      ...
      {isLoading ? (
        ...
        <DataTable
          data={auditLogs}
          columns={columns}
          searchKeys={['action', 'userId', 'userEmail', 'resource']}
          pageSize={15}
