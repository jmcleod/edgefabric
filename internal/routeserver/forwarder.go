...
func (s *ForwarderService) startRoute(rwg route.RouteWithGateway) (*routeRuntime, error) {
	...
	case domain.RouteProtocolICMP:
		if err := s.startICMPRoute(r, rwg.GatewayWGIP); err != nil {
			s.logger.Warn("ICMP route failed to start, skipping",
...
func (s *ForwarderService) startICMPRoute(r *domain.Route, gatewayWGIP string) error {
	var initErr error
	s.icmpOnce.Do(func() {
	...
	s.icmpProxy.addRoute(r.EntryIP, r.ID.String(), gatewayWGIP, r.DestinationIP)
	return nil
}
