...
func (s *PostgresStore) CreateAPIKey(ctx context.Context, k *domain.APIKey) error {
	...
	if err != nil {
		if isUniqueViolation(err) {
			return fmt.Errorf("%w: api key already exists", storage.ErrAlreadyExists)
		}
...
func (s *PostgresStore) GetAPIKey(ctx context.Context, id domain.ID) (*domain.APIKey, error) {
	...
	err := s.db.QueryRowContext(ctx,
		`SELECT id, tenant_id, user_id, name, key_hash, key_prefix, role, expires_at, last_used_at, created_at
		 FROM api_keys WHERE id = $1`, id.String(),
	).Scan(&k.ID, &tenantID, &userID, &k.Name, &k.KeyHash, &k.KeyPrefix,
...
func (s *PostgresStore) ListAPIKeys(ctx context.Context, tenantID domain.ID, params storage.ListParams) ([]*domain.APIKey, int, error) {
	...
	if err := s.db.QueryRowContext(ctx,
		`SELECT COUNT(*) FROM api_keys WHERE tenant_id = $1`, tenantID.String(),
	).Scan(&total); err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&k.ID, &tid, &uid, &k.Name, &k.KeyHash, &k.KeyPrefix,
...
func (s *PostgresStore) DeleteAPIKey(ctx context.Context, id domain.ID) error {
	result, err := s.db.ExecContext(ctx, `DELETE FROM api_keys WHERE id = $1`, id.String())
	if err != nil {
