...
// After identity resolution, the gateway persists its identity (gateway ID,
// API token) so it can reconnect to the controller on restart.
...
const stateFileName = "gateway-state.json"

// State holds the gateway's persisted identity.
type State struct {
